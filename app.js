...
// Complete the exercise in the space below:
let inventorNames = inventors.map((inv)=>{
...
// Complete the exercise in the space below:
let sortedByBirthYear = inventors.sort((first, second) => {
  if (first.year < second.year) return -1;
  if (first.year > second.year) return 1;
  return 0; // if years are equal
});
...
// Complete the exercise in the space below:
let inventorNamedAda = inventors.find((inv)=>{
    return inv.first=='Ada'
});
...
// Complete the exercise in the space below:
let idx = comments.findIndex((com)=>{
...
// Complete the exercise in the space below:
let totalYearsLived = inventors.reduce(function(acc,inv){
    let age=inv.passed-inv.year;
    return acc+age;
},0)
